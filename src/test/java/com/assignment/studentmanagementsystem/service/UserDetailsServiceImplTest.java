...
// File path: src/test/java/com/assignment/studentmanagementsystem/service/UserDetailsServiceImplTest.java
...
class UserDetailsServiceImplTest {
    ...
    void loadUserByUsername_found_returnsUserDetails() {
        UserAccount user = new UserAccount("student@test.com", "encoded", UserAccount.Role.STUDENT);
        when(managementRepository.findUserByUsername("student@test.com")).thenReturn(Optional.of(user));

        UserDetails result = userDetailsService.loadUserByUsername("student@test.com");
    ...
    void loadUserByUsername_notFound_throwsUsernameNotFoundException() {
        when(managementRepository.findUserByUsername("ghost@test.com")).thenReturn(Optional.empty());
    ...
    void loadUserByUsername_teacher_hasTeacherRole() {
        UserAccount user = new UserAccount("teacher@test.com", "encoded", UserAccount.Role.TEACHER);
        when(managementRepository.findUserByUsername("teacher@test.com")).thenReturn(Optional.of(user));

        UserDetails result = userDetailsService.loadUserByUsername("teacher@test.com");

        assertThat(result.getAuthorities()).anyMatch(a -> a.getAuthority().equals("ROLE_TEACHER"));
    }
}
