...
// File path: src/test/java/com/assignment/studentmanagementsystem/controller/RegistrationControllerIntegrationTest.java
...
class RegistrationControllerIntegrationTest {
    ...
    void postRegisterStudent_success_redirectsToLogin() throws Exception {
        when(userService.registerUser(eq("newstudent@test.com"), eq("pass123"), eq(Role.STUDENT)))
            .thenReturn(null);
    ...
    void postRegisterTeacher_success_redirectsToLogin() throws Exception {
        when(userService.registerUser(eq("newteacher@test.com"), eq("pass123"), eq(Role.TEACHER)))
            .thenReturn(null);
    ...
    @Test
    void postRegisterStudent_duplicateUsername_returnsFormWithError() throws Exception {
        when(userService.registerUser(any(), any(), eq(Role.STUDENT)))
    ...
    @Test
    void postRegisterTeacher_duplicateUsername_returnsFormWithError() throws Exception {
        when(userService.registerUser(any(), any(), eq(Role.TEACHER)))
            .thenThrow(new RuntimeException("Username already exists"));
